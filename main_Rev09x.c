...
unsigned int cal_corr_CH0, cal_corr_CH1, ac_cal_corr_CH0, ac_cal_corr_CH1; 
unsigned int cal_corr_CH0_51, cal_corr_CH1_51, ac_cal_corr_CH0_51, ac_cal_corr_CH1_51;
unsigned int cal_corr_CH0_80, cal_corr_CH1_80, ac_cal_corr_CH0_80, ac_cal_corr_CH1_80;
unsigned int cal_corr_CH0_10, cal_corr_CH1_10, ac_cal_corr_CH0_10, ac_cal_corr_CH1_10;
// </editor-fold>
 ...
 */
   ...
   // <editor-fold defaultstate="collapsed" desc="All initialize">
    ...
    for (i = 0; i < 524288; i++) {};
    Cal_values();                               //Read calibration values and apply them
    _RB13 = 1;                                  //Turn ON LCD
    ...
    while(1){
        ...
        if ((!menu_pb && menu_dwn) && (selection != 18) && (flags != 4)) {             // S2 jump to the main menu if S2 is pressed            
            ...
            while (!menu_pb){   //MENU PB is pressed, current is selected
                //ClrWdt();                       // To prevent a WDT Time-out Reset <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                // <editor-fold defaultstate="collapsed" desc="Current Test... Use Left Input  ">
                ...
                // </editor-fold>
                mydelay_sec(1);     // To slow down printing on LCD
            }
            ...
            // </editor-fold>
                ...
                // </editor-fold>
skip_main:
                while ((!menu_pb) || (!menu_dwn) || (!menu_up)){} // wait until user releases a button
...
skip:               
        ...
        switch( selection )
        {
            case 1: case 2:         // selection ==1 or ==2                     
                //ClrWdt();                   // To prevent a WDT Time-out Reset <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                ...
                // <editor-fold defaultstate="collapsed" desc="range_test()- Runs only when both dividers are not 0. Enacts voltage measurements. Resets dividers. Updates accumulators. No display.">
                /*
                 * Below:
...
//#define API
#ifdef API            
                range_test(); //This function will run only if both dividers are not zero.
#else            
                if((dividerCH0!=0)&&(dividerCH1!=0)){
                    ...
                    IEC0bits.OC2IE  = 0;    //OC2 is used to send data to PC thru UART TX// </editor-fold>

                    voltage_val = data_comp_CH0(vflag, rng_srch, v_smple_sum, voltage_sum_n, voltage_sum_p, cal, dividerCH0, cal_corr_CH0, ac_cal_corr_CH0);
                    current_val = data_comp_CH1(cflag, rng_srch, c_smple_sum, current_sum_n, current_sum_p, cal, dividerCH1, cal_corr_CH1, ac_cal_corr_CH1);

                    if (voltage_acc == 0) {//Accumulator gets reset only when a divider is reset
                        voltage_acc = voltage_val; //voltage_acc is in nV. If accumulator is in reset save currently measured value.//5/20/13
                    } else {//If voltage_acc !=0 it will contain the average value of the current reading
                        voltage_acc = (voltage_val + voltage_acc) / 2; //voltage_acc is in nV
                    }
                    if (current_acc == 0) {
                        current_acc = current_val;                  //5/20/13.
                    } else {
                        current_acc = (current_val + current_acc) / 2;                
                    }
                    
                    dividerCH0 = TstAndResetVoltDivCH0(dividerCH0,voltage_acc,voltage_val, selection, vflag);                                                 
                    dividerCH1 = TstAndResetVoltDivCH1(dividerCH1,current_acc,current_val, cflag);
                ...
                if ((vflag == 0x07) && (CH0_disp_frz == Disable) && (selection != 2)){   // If AC voltage detected and channel is not frozen
                ...
                if ((dividerCH0 == 0) || (dividerCH1 == 0)) {
                    ...
                    while ((!menu_up) && (freez_En == Disable)) {   //Freeze display.
                    ...
                    while ((!menu_up) && (freez_En == Enable)) {    //Unfreeze display
                        // <editor-fold defaultstate="collapsed" desc="'Menu up' unfreeze display. Push button controlled. Dependant on: (!menu_up) && (CH0_disp_frz == 1)">
                        //Unfreeze display
                        for (i = 0; i < 124288; i++) {
                        } //menu_up Press and Hold delay
                    ...
                    /*
                     * If one or both dividers are reset to zero, find new range and run numbers on the LCD
                ...
                if (selection == 2){
                    if (flags != 22){           // Set in Isetup                                   
                        Isetup();               // Sets dividerCH0 = 1;
                        _LATA9 = 1;             // Enable current read on CH0                                  
                    }
                    overcurrent();              // Continuously test for overcurrent. Take voltage reads on CH1 Left  
                    Imeasure();                 // Computes current
                }
...
//              }
        ...
        // </editor-fold>
           ...
           case 15:
                 ...
                 * Prior to using data-logger function configuration has to be downloaded from PC.
                 */
...
//                }
            ...
            case 31:    // Digital Sniffer Status Print                
                ...
                ClrWdt();
                // Global: volatile unsigned int T1InrrptCntr;
                T1InrrptCntr = T1InrrptCntr + 1;                    // To avoid zero  
                if (((T1InrrptCntr%39999) == 0) || (LAcntr > 6140)){
            ...
            case 32:
                ...
                while (!buffer_full) {}     //Wait until all data is collected
                // </editor-fold>               
                voltage_val = data_comp_CH0(vflag, rng_srch, v_smple_sum, voltage_sum_n, voltage_sum_p, cal, dividerCH0, cal_corr_CH0, ac_cal_corr_CH0); //(nV) //Returns average voltage value in nV
                current_val = data_comp_CH1(cflag, rng_srch, c_smple_sum, current_sum_n, current_sum_p, cal, dividerCH1, cal_corr_CH1, ac_cal_corr_CH1); //(nV) //Returns average voltage value in nV
               
                /*
                 * Input voltage dividers are set in DataloggerConfig.c. The display values are true for a particular
                 * input range setting.
                 */
                
                
                if (dividerCH0 == 1){                    
                    DataDisply (voltage_val, 0x80, vflag, voltage_sum_p, voltage_sum_n, CH0vshft, CH0vsftSgn);
                }
                ...
                if (dividerCH1 == 1){                    
                    DataDisply (current_val, 0xc0, cflag, current_sum_p, current_sum_n, CH1vshft, CH1vsftSgn);
                }
